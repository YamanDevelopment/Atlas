import { fetchCombinedUCFEvents } from './combinedEvents.js';
import { writeFileSync } from 'node:fs';

async function saveUCFEventsData() {
	try {
		console.log('📡 Fetching combined UCF events data...\n');
		
		const combinedData = await fetchCombinedUCFEvents();
		
		// Save to JSON file
		const fileName = `ucf-events-${new Date().toISOString().slice(0, 10)}.json`;
